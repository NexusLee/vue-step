<template>
  <div class="vux-step-item" :class=" { 'vux-step-item-with-tail' : !props_stepLast} ">
    ...
    <div :class="'vux-step-item-main ' + 'vux-step-item-main-' + props_status" :style="{/*backgroundColor: $parent.backgroundColor, paddingRight: props_stepLast ? 0 : $parent.gutter*/}" @click="toggleList">
      <span class="vux-step-item-title">
        {{title}}<i v-show="props_value.length > 0" :class="show ? 'fa fa-angle-up' : 'fa fa-angle-down'"></i>
      </span>
      <!--<div class="vux-step-item-description">{{description}}</div>-->
      <transition name="step-item-move">
        <div v-show="show">
          <div class="vux-step-item-value" v-for="(item, index) in props_value">
            <p>{{item}}</p>
          </div>
        </div>
      </transition>
